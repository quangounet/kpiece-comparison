...
DensoStateSpace::DensoStateSpace(RobotBasePtr probot)
{
    _probot = probot;
    ...
    // Add subspaces to DensoStateSpace
    addSubspace(ob::StateSpacePtr(qSpace), 0.5);
    addSubspace(ob::StateSpacePtr(qdSpace), 0.5);
}

unsigned int DensoStateSpace::getDimension()
{
    return _dimension;
}

void DensoStateSpace::registerProjections()
{
    registerDefaultProjection(ob::ProjectionEvaluatorPtr
			      (new DensoProjection(this, _probot, projectionType)));
}

double DensoStateSpace::distance(const ob::State *state1, 
				 const ob::State *state2) const
{
    const ob::CompoundStateSpace::StateType *s1 = 
	state1->as<ob::CompoundStateSpace::StateType>();
    const ob::CompoundStateSpace::StateType *s2 = 
	state2->as<ob::CompoundStateSpace::StateType>();
